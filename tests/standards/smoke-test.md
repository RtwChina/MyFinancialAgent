...
| SM-022  | 账户管理活态计划 CRUD | 验证账户管理页可创建 / 编辑 / 删除账户活态操作计划，并立即可读回 | migration 023 已 apply，账户存在且 enabled=1，tracked_symbols 含目标 symbol | 1. 打开“账户管理”页 2. 在某账户下「新增操作计划」填写 symbol/动作/计划字段并保存 3. 编辑该计划修改某字段并保存 4. 删除该计划 5. 重新 `GET /api/account-live-action-plans` | 各操作返回 2xx；列表回查与最近一次写入一致；重复 `(account_id, symbol)` POST 返回 409；详见 `tests/smoke/account_live_action_plans_crud.md` | P0 | 是 |
| SM-023  | 凌晨初始化复制活态计划到当日 | 验证 init 在当天 daily plans 为空时从 live 完整复制；非空时跳过 | migration 023 已 apply，`account_live_action_plans` 至少有 1 条，目标日 `daily_review_action_plans` 为空 | 1. 清空目标日 `daily_review_action_plans` 2. `POST /api/reviews/<date>/initialize` 3. 查询当日 plans 4. 再次调用同 endpoint | 第 3 步当日 daily plans 数量 = live 数量，逐字段相等；第 4 步不新增、不修改；symbol 已 `is_active=0` 的 live 行在复制时跳过并出现 warning；详见 `tests/smoke/review_init_copies_live.md` | P0 | 是 |
| SM-024  | 最新复盘日编辑回流活态 | 验证在 `archive_date == latestArchiveDate` 的抽屉里保存 plans 会同步覆盖 `account_live_action_plans` | migration 023 已 apply，最新 archive_date 存在 | 1. 打开最新日抽屉 2. 编辑/新增/删除某 plan 3. 保存 4. 查询 `account_live_action_plans` | 第 4 步 live 行与保存后的 daily plans 完全一致（DELETE+INSERT 整集替换语义）；historical 日（非 latest）执行同样操作不会改 live；详见 `tests/smoke/review_latest_day_syncs_live.md` | P0 | 是 |
| SM-025  | 资金分布汇总 | 验证统计汇总按币种、账户和标的汇总当前活态计划资金 | migration 023/024 已 apply，测试环境存在启用账户，`account_live_action_plans` 可写入 `position_amount` | 1. 创建 USD/CNY 测试账户和活态计划，含一条 `0%` 仓位 2. 请求 `GET /api/account-fund-summary` 3. 打开“统计汇总”页 4. 查看估算金额 `*` tooltip 与美元区人民币参考值 | API 返回 USD/CNY 独立分组；手填金额 source=exact，缺失金额按仓位区间中位数 source=estimated，`0%` 持仓标的不展示；同一标的跨账户合并；页面展示币种区、账户列、`*` tooltip；USD 摘要、账户覆盖和标的总金额显示人民币参考值，标题 tooltip 说明固定汇率 6.80，且无 `备注`/`来源` 列；不依赖券商/基金/实时汇率第三方接口 | P0 | 是 |
| SM-026  | 操作计划支撑压力导出 | 验证个股与资产操作计划可将支撑位、压力位转成 `draw_zone` 命令 | 测试环境存在含 `supportLevels` / `resistanceLevels` 的结构化操作计划 | 1. 打开复盘抽屉并进入“个股操作” 2. 点击“导出画线” 3. 检查预览内容 4. 使用无可解析区间的数据再次导出 | 支撑与压力按每个可解析区间各生成一行；参数顺序为 symbol、下限、上限、标签；中文括号与横线被规范化，力度和备注保留；无效行跳过；无可导出内容时显示空状态且复制按钮禁用；详见 `tests/smoke/action_plan_zone_export.spec.js` | P0 | 是 |

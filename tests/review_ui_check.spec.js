...
test('review workspace renders table and step modal', async ({ page, request }) => {
  ...
  await expect(page.locator('textarea[name="marketSentiment"]')).toBeHidden();
  ...
  await expect(page.locator('textarea[name="marketSentiment"]')).toBeVisible();
  await page.locator('textarea[name="marketSentiment"]').fill('美元和利率相对稳定，风险偏好仍偏向 AI 主线。');
  await page.locator('#nextStepBtn').click();
  await page.locator('textarea[name="sectorRotation"]').fill('通信设备和 AI 基建链更强，防御资产偏弱。');
  await page.locator('#nextStepBtn').click();

...
新闻处理采用三级漏斗：先用低成本规则快速打分，再用 Embedding 做语义过滤，最后才把候选新闻送入 LLM 深度分析。这样做的目的很简单：LLM 最贵最慢，不能把所有原始新闻直接丢进去。
...
Stage 1 成本最低，Stage 3 成本最高。前两层的存在，就是为了把昂贵的 LLM 使用集中到真正值得分析的候选新闻上。

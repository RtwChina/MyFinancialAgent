...
### Requirement: repair-prices 按市场分流备用修复源

系统 SHALL 在 `repair-prices` 任务中使用 `Yahoo` 作为第一优先修复源；当 Yahoo 未能返回目标 `k_date` 的有效价格时，系统 SHALL 按市场分流到对应的二级 fallback：`.SS` / `.SZ` 使用 `AKShare`，`.HK` 与国际链路标的使用 `Finnhub`。
...
#### Scenario: `.HK` 标的 Yahoo 缺目标日线，Finnhub 命中

- **GIVEN** `stock_raw` 中存在一条坏记录，`yahoo_symbol = 3067.HK`、`k_date = 2026-04-02`、`current_price = NULL`
- **WHEN** `repair-prices` 任务先查询 Yahoo，且 Yahoo 未返回 `2026-04-02` 的价格
- **THEN** 系统继续使用 Finnhub 查询该标的的 `2026-04-02`
- **AND** 若 Finnhub 返回 `2026-04-02` 的非空价格，则系统允许修复该记录

### Requirement: 备用源也必须命中同一 k_date

系统 SHALL 仅在 AKShare 或 Finnhub 返回与候选记录相同 `k_date` 的非空价格时，才认定 fallback 修复成功。
...
#### Scenario: Finnhub 返回错日数据

- **GIVEN** `stock_raw` 中存在一条坏记录，`yahoo_symbol = 3067.HK`、`k_date = 2026-04-02`、`current_price = NULL`
- **WHEN** Finnhub 只能返回 `2026-04-01` 的价格
- **THEN** 系统不得将该结果写回到 `2026-04-02` 的记录

### Requirement: 修复日志必须区分来源与失败阶段

系统 SHALL 在 `repair-prices` 任务中记录修复来源与关键失败阶段，至少区分 Yahoo 命中、Yahoo 缺目标日线后转 AKShare / Finnhub、AKShare 命中/失败、Finnhub 命中/失败。
...
#### Scenario: Yahoo miss 后 Finnhub 命中

- **GIVEN** 一条 `.HK` 或国际链路坏记录需要修复
- **WHEN** Yahoo 未命中目标 `k_date`，但 Finnhub 命中并修复成功
- **THEN** 日志中必须能看出该记录经历了 `Yahoo miss -> Finnhub hit`

...
def _call_batch_llm(news_batch: List[Dict[str, Any]], batch_id: str) -> Dict[str, Any]:
    ...
    five_star_cap = max(1, math.ceil(batch_count * 0.05))
    four_star_cap = max(1, math.ceil(batch_count * 0.10))
    high_star_cap = max(1, math.ceil(batch_count * 0.20))
    messages = [
        {
            "role": "system",
            "content": (
                ...
                "评分必须严格参照以下标准和示例：\n\n"
                "**5星（极少数，改写当日交易主线）**：只给极少数真正改写当日交易主线的事件。"
                "必须明显改变宏观主线、大盘方向、流动性/利率预期，或直接改变核心跟踪标的的一阶定价逻辑。\n"
                "  示例：「美联储意外降息50基点，远超市场预期的25基点」\n\n"
                "**4星（少数，复盘重点输入）**：只给少数值得写入当天复盘重点段落的事件。"
                "必须有明确新增事实，且对重点板块、核心资产或跟踪标的有较强、可落到交易层面的影响。\n"
                "  示例：「美光科技Q3财报：营收同比增长93%，HBM出货量超预期30%」\n\n"
                "**3星（主体，有信息增量）**：有明确市场信息增量、值得纳入复盘，但不属于当天最核心输入层。\n"
        ...
        },
        {
            ...
            "content": (
                ...
                f"   - 5星：最多{five_star_cap}条（最多本批新闻的5%）\n"
                f"   - 4星：最多{four_star_cap}条（最多本批新闻的10%）\n"
                f"   - 4-5星合计：最多{high_star_cap}条（最多本批新闻的20%）\n"
                "   - 这些是硬约束，不是建议值。如果你觉得更多新闻应得到4-5星，说明你的评分标准过宽，必须重新校准。\n"
                "7. 若多条新闻属于同一主线事件，只有最关键、最完整、最具决定性的少数新闻可以给 4-5 星；其余后续重复快讯原则上降为 2-3 星。\n"
                "8. 若新闻没有新增事实，只包含评论、媒体解读、盘中涨跌描述、评级目标价、媒体转述或摘要汇编，通常不得高于 3 星。\n"
                "9. 若新闻直接影响美联储路径、利率/通胀预期、地缘风险、核心指数，或核心跟踪标的的财报、指引、订单、监管、资本开支，通常应优先考虑 3-5 星。\n"
                "10. 如果只是“重要”但没有改写当日主线，不得给 5 星；如果只是重要补充但不属于最核心输入，应优先归入 3 星。\n"
                "11. 只返回 JSON。\n\n"

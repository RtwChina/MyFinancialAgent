...
from data_sources.price_live import fetch_price_for_k_date_live
...
def run_price_repair(context: ExecutionContext | None = None, days: int = RECENT_REPAIR_DAYS) -> dict[str, int]:
    ...
    for candidate in candidates:
        ...
        logger.info("开始修复候选: symbol=%s yahoo=%s k_date=%s", symbol, yahoo_symbol, target_k_date)

        repaired = fetch_price_for_k_date_live(candidate, context)
        if not repaired:
            stats["skipped"] += 1
        ...
        if updated:
            stats["repaired"] += 1
            logger.info(
                "修复成功: symbol=%s yahoo=%s k_date=%s price=%s",
                symbol, yahoo_symbol, target_k_date, repaired.get("current_price"),
            )
        else:
            stats["failed"] += 1
            logger.warning("修复未更新任何记录: symbol=%s yahoo=%s k_date=%s", symbol, yahoo_symbol, target_k_date)
